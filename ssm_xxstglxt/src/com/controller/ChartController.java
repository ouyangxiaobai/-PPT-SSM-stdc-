...
public class ChartController extends BaseController {
	...
	@RequestMapping("chartlineSeller.action")
	@ResponseBody
	public String chartlineSeller() throws JSONException {
		String start = this.getRequest().getParameter("start");
		String end = this.getRequest().getParameter("end");
		String sellerid = this.getRequest().getParameter("sellerid");
		long days = VeDate.getDays(end, start) + 1;
		JSONArray total = new JSONArray();
		JSONArray count = new JSONArray();// 定义count存放数值
		JSONArray day = new JSONArray(); // 存放名称
	...
	@RequestMapping("chartPieSeller.action")
	@ResponseBody
	public String chartPieSeller() throws JSONException {
		String sellerid = this.getRequest().getParameter("sellerid");
		JSONArray names = new JSONArray();
		JSONArray count = new JSONArray();// 定义count存放数值
		int stars1 = 0;
		...
		List<Topic> topicList = this.topicService.getTopicByCond(x);
		for (Topic topic : topicList) {
			if ("1".equals(topic.getNum())) {
				stars1++;
			}
