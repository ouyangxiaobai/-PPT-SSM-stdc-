...
<body>
	<nav class="breadcrumb">
		<i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span>商户管理 <span class="c-gray en">&gt;</span> 商户查询 <a
	...
	<div class="page-container">
		<table class="table table-border table-bordered table-bg">
			<thead>
				<tr>
					<th scope="col" colspan="10">商户查询</th>
				</tr>
				<tr class="text-c">
					...
					<th class="center">商铺地点</th>
					<th class="center">开业日期</th>
			...
			<c:forEach items="${sellerList}" var="seller">
				<tr class="text-c">
					<td class="center">${seller.username}</td>
					<td class="center">${seller.shopname}</td>
					<td class="center">${seller.idcard}</td>
					<td class="center">${seller.contact}</td>
					<td class="center">${seller.address}</td>
					<td class="center">${seller.workdate}</td>
					<td class="center">${seller.status}</td>
					<td class="center">${seller.regdate}</td>
				</tr>
			</c:forEach>
		</table>
		<div class="cl pd-5 mt-20"></div>
		<div class="text-c">
			<form action="seller/querySellerByCond.action" name="myform" method="post">
				查询条件： <span class="select-box" style="width: 200px;"><select name="cond" class="select">
						...
						<option value="shopname">按商铺名称查询</option>
						<option value="idcard">按身份证查询</option>
						<option value="contact">按联系方式查询</option>
						<option value="address">按商铺地点查询</option>
						<option value="workdate">按开业日期查询</option>
